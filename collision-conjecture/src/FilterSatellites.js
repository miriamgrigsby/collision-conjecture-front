   ...
   const handleChange = (event) =>{
    ...
    const handleSubmit = (event) => {
        ...
        const matchPositions = positions.find(position => {
            return event.target.elements[0].value.toUpperCase() === position.name.toUpperCase() ||  event.target.elements[0].value.toUpperCase() === position.country.toUpperCase() ? position : "wtf)"
        })
